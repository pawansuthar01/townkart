...
export default function CustomerLocationPage() {
  ...
  useEffect(() => {
    ...
    const checkPermissions = async () => {
      try {
        ...
        console.log("Customer location permission status:", status);
    ...
    const checkTrackingStatus = async () => {
      try {
        const response = await fetch("/api/user/location/status");
        if (response.ok) {
  ...
  const getCurrentLocation = async () => {
    setIsLoading(true);
    setError(null);
    console.log("Getting customer current location...");
    ...
    try {
      ...
      console.log("Customer location received:", locationData);
  ...
  const toggleLocationTracking = async () => {
    try {
      ...
      const response = await fetch("/api/user/location/toggle", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const requestLocationPermission = async () => {
    try {
      setIsLoading(true);
      setError(null);
      console.log("Requesting customer location permission...");

      // Request permission explicitly
      const status = await locationService.requestPermission();
      console.log("Customer permission request result:", status);
      setPermissionStatus(status);

      if (status.granted) {
        console.log("Customer permission granted, getting location...");
        ...
        console.log(
          "Customer permission status unclear, trying to get location anyway..."
        );
        // Try to get location even if status is unclear
        await getCurrentLocation();
      }
    } catch (error) {
      console.error("Customer permission request failed:", error);
